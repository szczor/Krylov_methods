...
[x1_left_M1, converged, ic_gmres_left_M1, rn_gmres_left_M1] = gmres_pre_left(A, b,m_gmres, max_iter, stop_res, M1);
[x1_left_M2, converged, ic_gmres_left_M2, rn_gmres_left_M2] = gmres_pre_left(A, b,m_gmres, max_iter, stop_res, M2);
[x1_left_M3, converged, ic_gmres_left_M3, rn_gmres_left_M3] = gmres_pre_left(A, b,m_gmres, max_iter, stop_res, M3);
...
xmin = 0;
xmax = max([ic_gmres_left_M1+1, ic_gmres_left_M2+1, ic_gmres_left_M3+1, ic_gmres_left_M4+1]) + 1;
ymin = min([min(rn_gmres_left_M1), min(rn_gmres_left_M2), min(rn_gmres_left_M3), min(rn_gmres_left_M4)]) * 0.8;
ymax = max([max(rn_gmres_left_M1), max(rn_gmres_left_M2), max(rn_gmres_left_M3), max(rn_gmres_left_M4)]) * 1.2;
axis([xmin xmax ymin ymax]);


xlabel('Liczba Iteracji'), ylabel('2 norma residuum'), hold on
legend('Jacobi', 'Gauss-Seidel', 'SOR(1/2)','ILU(0)'), hold on
title_str1 = 'GMRES z lewym preconditioningiem dla jednowymiarowego zadania';
title({title_str1}), hold off
saveas(gcf,'C:\Users\rysza\Desktop\PMPMPMPM\implementation\Plots\preconditioned\left_preconditioned_problem1.jpg')